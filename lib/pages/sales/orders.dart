...
import 'package:poss_mobile_app/pages/sales/receipt_image_generator.dart';
import 'package:poss_mobile_app/services/web_socket_service.dart';
...
// Note the dot prefix for broadcastAs() events
reverb.on('.order.added', (data) {
  print('New order: $data');
  toast("New order received");
});

// Connect — pending channels are flushed automatically after handshake
reverb.connect();
reverb.subscribePrivate('orders'); // becomes "private-orders" automatically
...
void dispose() {
                  ...
                  Expanded(
                    child: _gradientButton(
                      ...
                      onTap: () {
                        ...
                        ReceiptImageGenerator.show(
                          context: context,
                          order:
                              order, // pass the local `order` variable directly
                          shop: widget.shop,
                        );
                      },
                    ),
  ...
  Future<void> _printReceipt(Map order) async {
    ...
    final String shopName = widget.shop['name']?.toString() ?? "POS SHOP";
                        ...
                        return pw.TableRow(
                          children: [
                            ...
                            pw.Padding(
                                padding: const pw.EdgeInsets.all(8),
                                child:
                                    pw.Text("${price.toStringAsFixed(0)} TZS")),
                            pw.Padding(
                                padding: const pw.EdgeInsets.all(8),
                                child: pw.Text(
